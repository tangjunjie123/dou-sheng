...
func (c consulServiceRegistry) Register(serviceInstance ServiceInstance) bool {
	...
	//	schema = "https"
	//}
	check.HTTP = fmt.Sprintf("%s://%s:%d/health", schema, registration.Address, registration.Port+1)
	check.Timeout = "5s"
	check.Interval = "5s"
...
func Register(serviceId, consulIp, registerIp string, registerPort, consulPort int) {
	registryDiscoveryClient, _ := NewConsulServiceRegistry(consulIp, consulPort, "")

	serviceInstanceInfo, _ := NewDefaultServiceInstance(
		serviceId, registerIp, registerPort,
	...
	err := r.Run(":" + strconv.Itoa(registerPort+1))
	if err != nil {
