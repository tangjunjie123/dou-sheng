...
func NewDefaultServiceInstance(
	serviceId string, host string, port int, secure bool,
	metadata map[string]string, instanceId string,
) (*DefaultServiceInstance, error) {
	...
	if len(instanceId) == 0 {
		instanceId = serviceId + "-" + strconv.FormatInt(
			time.Now().Unix(),
			10,
		) + "-" + strconv.Itoa(rand.Intn(9000)+1000)
	}
